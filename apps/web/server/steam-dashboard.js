...
function decodeXmlValue(value) {
  ...
  const withoutCdata = trimmed.replace(/^<!\[CDATA\[([\s\S]*?)\]\]>$/i, '$1')
  ...
  return gameBlocks
    .map((gameXml) => {
      ...
      return {
        ...
        playtimeMinutes: parseHoursToMinutes(
          readXmlTag(gameXml, 'hoursOnRecord')
        ),
...
async function fetchStoreDetails(appId) {
  ...
  try {
    ...
    return {
      headerImage:
        typeof data.header_image === 'string' ? data.header_image : '',
      shortDescription:
        typeof data.short_description === 'string'
          ? data.short_description
          : '',
      releaseDate:
        typeof data.release_date?.date === 'string'
...
function mergeFeaturedWithLiveGames(
  staticFeatured,
  liveGamesByAppId,
  achievementMap
) {
  ...
  const fallbackFeatured = sortGamesByPlaytime(
    Array.from(liveGamesByAppId.values())
  )
    .filter((game) => game.playtimeMinutes > 0)
...
export async function createSteamStaticLibrary(config = getSteamConfig()) {
  ...
  const gamesWithAchievements = mergeAchievementStatsIntoGames(
    games,
    achievementMap
  )
  const featured = await enrichFeaturedGames(
...
export async function createSteamLiveDashboard(
  ...
  const staticGames = Array.isArray(staticLibrary?.games)
    ? staticLibrary.games
    : []
  ...
  const staticGamesByAppId = new Map(
    staticGames.map((game) => [game.appid, game])
  )
  const mergedGames = mergeStaticAndLiveGames(staticGames, liveGames)

  const changedGames = liveGames.filter((game) =>
    shouldRefreshAchievementStats(
      staticGamesByAppId.get(game.appid) ?? null,
      game
    )
  )
