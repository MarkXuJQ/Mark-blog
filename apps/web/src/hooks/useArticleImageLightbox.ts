 ...
 */
export function useArticleImageLightbox(deps: unknown[] = []) {
  ...
  useEffect(() => {
    ...
    const zoomableImages = images.filter(
    ...
    zoomableImages.forEach((img, index) => {
      ...
      if (
        ...
        sibling.classList.contains('img-caption')
      ) {
        sibling.remove()
      }
      ...
      if (altText) {
        ...
        caption.className =
          'img-caption block text-center text-sm text-slate-500/80 dark:text-slate-400/80 mt-2 mb-0 font-medium'
    ...
    return () => {
      handlers.forEach(({ element, listener }) => {
        ...
        if (
          ...
          figure.classList.contains('post-image-figure')
        ) {
