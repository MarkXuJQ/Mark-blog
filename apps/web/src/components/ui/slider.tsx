...
>(({ className, showTooltip = false, tooltipContent, ...props }, ref) => {
  ...
  const renderThumb = (value: number) => {
    const thumb = (
      <SliderPrimitive.Thumb
        className="block h-5 w-5 rounded-full border-2 bg-[var(--page-background)] shadow-[0_8px_20px_-12px_rgba(15,23,42,0.7)] transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-black/20 data-[disabled]:cursor-not-allowed dark:focus-visible:outline-white/25"
  ...
  return (
    <SliderPrimitive.Root
      ...
      {...props}
    >
      <SliderPrimitive.Track className="relative grow overflow-hidden rounded-full bg-black/10 data-[orientation=horizontal]:h-2 data-[orientation=horizontal]:w-full data-[orientation=vertical]:h-full data-[orientation=vertical]:w-2 dark:bg-white/14">
