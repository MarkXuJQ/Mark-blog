...
export function ThemeCurtain({ state }: ThemeCurtainProps) {
  ...
  const isTransitioningToDark =
    state.fromTone === 'light' && state.toTone === 'dark'
  const isTransitioningToLight =
    state.fromTone === 'dark' && state.toTone === 'light'
  const transitionTimingFunction =
    state.phase === 'exit'
      ? 'cubic-bezier(0.55, 0, 0.1, 1)'
      : 'cubic-bezier(0.2, 0.82, 0.18, 1)'

  const starOpacity = isTransitioningToDark
    ? state.phase === 'enter'
      ? 0
      : state.phase === 'cover'
        ? 0.72
  ...
  return (
    ...
    >
      <div
        className={cn(
          'absolute inset-x-0 top-0 h-[115svh] overflow-hidden transition-[opacity,transform] will-change-[opacity,transform]'
        )}
        style={{
