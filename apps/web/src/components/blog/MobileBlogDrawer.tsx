...
const styles = {
  ...
  drawer:
    'fixed inset-y-0 left-0 z-[90] flex w-[min(72vw,300px)] min-w-[260px] flex-col overflow-hidden border-r border-slate-200 bg-white shadow-2xl lg:hidden dark:border-[var(--border-color)] dark:bg-[var(--surface-card)]',
  profile:
  ...
  avatarWrapper:
    'relative z-10 mb-3 overflow-hidden rounded-full border-4 border-slate-100 shadow-sm dark:border-[var(--border-color)]',
  ...
  navLink: (isActive: boolean) =>
    cn(
      ...
      isActive
        ? 'bg-slate-100 text-slate-950 dark:bg-[color-mix(in_srgb,var(--surface-card)_78%,var(--text-primary)_10%)] dark:text-[var(--text-primary)]'
        : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950 dark:text-[var(--text-secondary)] dark:hover:bg-[color-mix(in_srgb,var(--surface-card)_78%,var(--text-primary)_10%)] dark:hover:text-[var(--text-primary)]'
    ),
  navIcon: 'text-blue-500',
  modePanel:
    'mx-4 mt-auto mb-5 flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 dark:border-[var(--border-color)] dark:bg-[color-mix(in_srgb,var(--surface-card)_84%,var(--page-background)_16%)]',
