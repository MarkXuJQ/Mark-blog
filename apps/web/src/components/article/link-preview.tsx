...
export function LinkPreviewCard({
  ...
  const previewMedia = imageLoadFailed ? (
    ...
    <img
      ...
      width={peekWidth}
      height={peekHeight}
      className="pointer-events-none block rounded-[5px] bg-neutral-50 align-top dark:bg-neutral-800"
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            {badge ? (
              <div className="text-[0.68rem] font-semibold tracking-[0.18em] text-slate-500 uppercase dark:text-slate-400">
                {badge}
              </div>
            ) : null}
            <div className="mt-1 line-clamp-2 text-[1rem] leading-6 font-semibold text-slate-950 dark:text-slate-50">
              {title}
            </div>
          </div>
          {href ? (
    ...
    >
      ...
      <RdxHoverCard.Portal>
        <RdxHoverCard.Content
          className="z-[120] [--radix-hover-card-content-transform-origin:center_center] [perspective:800px]"
          side="top"
          align="center"
          sideOffset={12}
