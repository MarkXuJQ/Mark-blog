...
export const referencePanelEmbed: ArticleEmbedDefinition = {
  name: 'ArticleReferences',
  selector: 'article-references, [data-article-references]',
  render({ document, source, context }) {
    ...
    section.setAttribute('data-article-embed', 'references')
    ...
    Array.from(source.querySelectorAll(':scope > a[href]')).forEach(
      (sourceLink) => {
