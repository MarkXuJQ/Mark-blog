...
function ProfileContent() {
  ...
  const handleSearch = (value: string) => {
    ...
    if (trimmed) {
      window.location.href = `/blog?q=${encodeURIComponent(trimmed)}`
    }
  }
...
function ArchiveContent() {
  ...
  return (
    <>
      <a href="/blog" className={cn(styles.widgetHeader, 'mb-4')}>
        <FileText size={20} className="text-blue-500" />
        <h3 className={styles.widgetTitle}>{t('nav.blog')}</h3>
      </a>
      <a href="/archive" className={styles.widgetHeader}>
...
const styles = {
  ...
  // Widget Shared
  widgetHeader: 'flex items-center gap-3 transition-opacity hover:opacity-80',
  widgetTitle: 'text-lg font-bold text-slate-800 dark:text-slate-100',

  // Stats
  statsGrid: 'mt-6 grid grid-cols-2 gap-4',
  statItem: 'flex items-center gap-3',
  statIconWrapper:
    'flex h-8 w-8 items-center justify-center rounded-lg bg-slate-50 text-slate-500 dark:bg-slate-800/50 dark:text-slate-400',
  statContent: 'flex flex-col',
  statValue: 'text-sm font-bold text-slate-800 dark:text-slate-100',
  statLabel: 'text-[10px] text-slate-500 uppercase tracking-wider dark:text-slate-500',
}
