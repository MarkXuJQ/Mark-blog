...
## 快速启动与学习路线（Phase 0: 入门与基建）

为了让你从 Python 平滑过渡到前端开发，我们将 M0 阶段细分为以下四个可执行步骤：

### Step 1: 基础概念与环境准备

- **目标**：理解前端“三件套”与现代框架的区别，准备开发环境。
- **概念映射**：
  - HTML/DOM ≈ 界面骨架
  - CSS ≈ 界面皮肤
  - JavaScript/TypeScript ≈ 交互逻辑（Python 脚本）
  - React 组件 ≈ Python 类/模块（封装数据与逻辑）
- **行动**：
  - 确保 Node.js 环境就绪（前端的 Python 解释器）。
  - 了解包管理器 `npm`/`pnpm`（前端的 pip）。

### Step 2: 初始化项目与工具链

- **目标**：使用现代工具生成项目骨架，理解目录结构。
- **工具**：
  - **Vite**：构建工具与开发服务器（类似 Python 的 Web 框架自带的 runserver，但更快）。
- **行动**：
  - 运行 `pnpm create vite` 生成 React + TypeScript 项目。
  - 对比分析项目结构（`src`, `public`, `package.json` vs `requirements.txt`）。

### Step 3: 构建初始界面

- **目标**：亲手写下第一行 React 代码，看到界面变化。
- **行动**：
  - 清理 Vite 生成的默认代码。
  - 创建一个简单的“Hello World”和“个人简介”组件。
  - 引入 **Tailwind CSS**，体验通过类名快速写样式（不再需要在 CSS 文件和 HTML 之间来回跳转）。

### Step 4: 运行与实时预览

- **目标**：掌握开发调试流程。
- **工具对比**：
  - **Vite Dev Server** vs **Live Server 插件**：
    - 我们将使用 Vite 自带的开发服务器（`npm run dev`）。它提供“热模块替换（HMR）”，保存代码后浏览器无需刷新即可局部更新，比传统的 Live Server 插件更适合 React 应用。
- **行动**：
  - 启动服务，在浏览器中查看效果。
  - 使用浏览器开发者工具（F12）查看元素与控制台日志。
